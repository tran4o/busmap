...
function blendColorToArray(color, alpha) 
{
	if (!color)
		return color;
	var parse = require('parse-color');
	var res = parse(color).rgba;
	res[3]*=alpha; // blend 
    return res;
}

function blendColor(color, alpha) 
{
	var parse = require('parse-color');
	var res = parse(color).rgba;
	res[3]*=alpha; // blend 
...
var RENDEREDARROWS={};
function renderArrow(width,height,color) 
{
	...
	+'<path fill="#605CC9" stroke="#FFFFFF" stroke-width="8" stroke-miterlimit="10" d="M239.5-36.8l-92.558-35.69 c5.216,11.304,8.13,23.887,8.13,37.153c0,12.17-2.451,23.767-6.883,34.327L239.5-36.8z"/>'
	+'</svg>'
    ...
    return RENDEREDARROWS[key]=img;
}

var RENDEREDDIRECTIONS={};
function renderDirection(width,height,color) 
{
	var key = width+"x"+height+":"+color;
    ...
    var img = document.createElement("img");
    img.src=canvas.toDataURL();
    return RENDEREDDIRECTIONS[key]=img;
}
...
exports.blendColorToArray=blendColorToArray;
...
exports.renderDirection=renderDirection;
