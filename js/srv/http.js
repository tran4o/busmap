...
var locCache = LRU({ max: 5000 });	   // TODO ADD OPTION in installation.xconf

function getGPSLocationName(data,onDone) 
{
	if (!data.lon || !data.lat) 
		return onDone(({ imei : data.imei,txt : 'Unknown location' }));
	...
	geocoder.reverse({lat:data.lat, lon:data.lon}, function(err, res) {
		if (err) {
		    ...
		    onDone(({ imei : data.imei,txt : 'Unknown location' }));
		    return;
		}
		var dres;
		if (res && res.length) for (var e of res) 
		{
			var arr=[];
			if (e.state) 
				arr.push(e.state);
			...
			{ 
				if (!isNaN(e.extra.distance_km)) 
					dres=({ imei : data.imei, txt : (Math.round(parseFloat(e.extra.distance_km)*100)/100.0) + "km from "+pfx+arr.join(" ")});
				else
					dres=({ imei : data.imei, txt : pfx+arr.join(" ")});
				break;
			} 
		}
		if (!dres)
			dres=({ imei : data.imei,txt : 'Unknown location' });
